...
class ActionDataBase implements ActionDataContract
{
    ...
    public static function createFromArray(array $parameters = []): self
    {
        ...
        $instance->user = auth()->check() ? auth()->user() : null;
        try {
    ...
    {
        if (!$this->validate()) {
            throw new OperationException($this->getValidationErrors()?->first(), OperationException::ERROR_INPUT_VALIDATION);
        }
    }
